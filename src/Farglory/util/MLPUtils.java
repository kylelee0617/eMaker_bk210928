...
public  class  MLPUtils  extends bvalidate{
    ...
    */
    ...
    /**
    * 查詢控管名單
    * 條件: 控管名單 C_Status = 'Y' or B_Status=Y
    */
    public Map getQueryLog() throws Throwable {
        String sql = "select name , query_id , C_Status , Project_id from QUERY_LOG where  C_Status = 'Y' or B_Status = 'Y' ";
        String[][]  retQueryLog = dbPW0D.queryFromPool(sql);
        Map qMap = new HashMap();
        if(retQueryLog.length > 0){
            for(int row=0 ; row<retQueryLog.length ; row++){
    ...
    public String[][] getDeputy(String docNo) throws Throwable {
        String sql = "select PaymentDeputy , DeputyName , DeputyID , DeputyRelationship from Sale05M080 where DocNo = '"+ docNo+"' and PaymentDeputy = 'Y' and (LEN(DeputyName)>0 or LEN(DeputyID)>0 or LEN(DeputyRelationship)>0) "
        + "union "
        + "select PaymentDeputy , DeputyName , DeputyID , DeputyRelationship from Sale05M082 where DocNo = '"+ docNo+"' and PaymentDeputy = 'Y' and (LEN(DeputyName)>0 or LEN(DeputyID)>0 or LEN(DeputyRelationship)>0) "
        + "union "
        + "select PaymentDeputy , DeputyName , DeputyID , DeputyRelationship from Sale05M083 where DocNo = '"+ docNo+"' and PaymentDeputy = 'Y' and (LEN(DeputyName)>0 or LEN(DeputyID)>0 or LEN(DeputyRelationship)>0) "
        + "union "
        + "select PaymentDeputy , DeputyName , DeputyID , DeputyRelationship from Sale05M328 where DocNo = '"+ docNo+"' and PaymentDeputy = 'Y' and (LEN(DeputyName)>0 or LEN(DeputyID)>0 or LEN(DeputyRelationship)>0) ";
    ...
    public String[][] getCtrlBeneficiary(List orderNo , String customNo) throws Throwable {
        ...
        for( int i=0 ;  i<cusNoOne.length ; i++ ){
            if( i != 0 ) sqlQ2_Cus.append(",");
            sqlQ2_Cus.append("'").append( cusNoOne[i] ).append("'");
        }

        String sql = "select BCustomNo , BenName  from Sale05M091ben T91 where OrderNo in ("+ sqlQ1.toString().trim() +") and CustomNo in ("+ sqlQ2_Cus.toString().trim() +")  and ISNULL(StatusCd, '') != 'C'  ";
    ...
    public String getBeneficiaryCtrlYN(String projectID , Map queryLog , String[][] arrBeneficiary , String mode) throws Throwable {
        ...
        for( int i=0 ; i<arrBeneficiary.length ; i++ ){
            ...
            if( "list".equals(mode) ) sb.append( arrBeneficiary[i][1].trim() );
            if( "ctrl".equals(mode) ) sb.append( arrBeneficiary[i][1].trim() + ":" + ("Y".equals( queryLog.get( thisMapKey ) )? "是":"否") );
        }
        return sb.toString();
    }
    ...
    /**
     * 實質受益人欄位檢核
     * 20200610 Kyle : 增加制裁名單先行檢核，檢核未過阻擋新增/修改，用amlCons是否為null控制是否檢核
     * 
     */
