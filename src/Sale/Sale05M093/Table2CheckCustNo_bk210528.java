...
public class Table2CheckCustNo_bk210528 extends bvalidate{
	public boolean check(String value)throws Throwable{
		...
		stringSQL = " SELECT TOP 1 auditorship, Nationality, CountryName, CustomName, Percentage, Birthday,MajorName, PositionName,ZIP,City,Town,Address,Cellphone,Tel,Tel2,eMail,IsBlackList, IsControlList, IsLinked "  + 
							  " FROM Sale05M091 "  +
							" WHERE CustomNo = '" + stringCustomNoNew + "' " +
		...
		for(int  intSale05M091=0  ;  intSale05M091<retSale05M091.length  ;  intSale05M091++){
				...
				//setValueAt("table2",  retSale05M091[intSale05M091][4].trim(),  intSelectRow,  "Percentage");		//比例
		...
		String sqlBlack = "Select TOP 1 B_STATUS, C_STATUS, R_STATUS,NAME,BIRTHDAY FROM QUERY_LOG WHERE QUERY_ID = '" + value + "' AND PROJECT_ID = '"+strProjectID1+"' Order By QID Desc ";
		...
		if(retBlack.length > 0) {
			...
			String str400sql = "";
			if("".equals(BirthDay)){
					str400sql = "SELECT * FROM CRCLNAPF WHERE CONTROLLISTNAMECODE IN (SELECT DISTINCT C.CONTROLLISTNAMECODE FROM CRCLNCPF C,CRCLCLPF L WHERE C.CONTROLCLASSIFICATIONCODE=L.CONTROLCLASSIFICATIONCODE AND L.CONTROLCLASSIFICATIONCODE ='X181') AND CUSTOMERID = '"+value+"' AND CUSTOMERNAME='"+logName+"' ";
			}else{
					str400sql = "SELECT * FROM CRCLNAPF WHERE CONTROLLISTNAMECODE IN (SELECT DISTINCT C.CONTROLLISTNAMECODE FROM CRCLNCPF C,CRCLCLPF L WHERE C.CONTROLCLASSIFICATIONCODE=L.CONTROLCLASSIFICATIONCODE AND L.CONTROLCLASSIFICATIONCODE ='X181') AND CUSTOMERID = '"+value+"' AND ( CUSTOMERNAME='"+logName+"' OR BIRTHDAY = '"+BirthDay+"' )";
			}
			...
			//制裁名單X171
			if("".equals(BirthDay)){
					str400sql = "SELECT * FROM CRCLNAPF WHERE CONTROLLISTNAMECODE IN (SELECT DISTINCT C.CONTROLLISTNAMECODE FROM CRCLNCPF C,CRCLCLPF L WHERE C.CONTROLCLASSIFICATIONCODE=L.CONTROLCLASSIFICATIONCODE AND L.CONTROLCLASSIFICATIONCODE ='X171') AND CUSTOMERID = '"+value+"' AND CUSTOMERNAME='"+logName+"' ";
			}else{
					str400sql = "SELECT * FROM CRCLNAPF WHERE CONTROLLISTNAMECODE IN (SELECT DISTINCT C.CONTROLLISTNAMECODE FROM CRCLNCPF C,CRCLCLPF L WHERE C.CONTROLCLASSIFICATIONCODE=L.CONTROLCLASSIFICATIONCODE AND L.CONTROLCLASSIFICATIONCODE ='X171') AND CUSTOMERID = '"+value+"' AND ( CUSTOMERNAME='"+logName+"' OR BIRTHDAY = '"+BirthDay+"' )";
			}
