...
public class ChgNameSave extends bproc{
  public String getDefaultValue(String value)throws Throwable{
    ...
    for(int  intNo=0  ;  intNo<retSale05M086.length  ;  intNo++) {
      ...
      //System.out.println((intNo+1)+"--------------------"+stringEDate+"-----------------"+datetime.subDays1(stringTrxDate,  stringEDate)) ;
    ...
    for (int  i=0  ;  i< tb2_string.length  ;  i++){
      ...
      if ("1".equals(stringChangeName)){
        ...
        //if(i == tb2_string.length-1  &&  Float.parseFloat(stringPercentSum)  !=  100) {
        //    message("[比例%] 合計須為 100 %。");
        //    jtable2.setRowSelectionInterval(i, i)  ;
        //    return value ;        
        //}
    ...
    if (mlpUtils.checkBenColumn( tb5_string , amlCons ) == false) return value;

    // 異動資料庫
    // 客戶資料
    stringSQL="SELECT  max(RecordNo) "  +
            " FROM  Sale05M091 "+
            " WHERE  OrderNo  =  '"  +  stringOrderNo  +  "' " ;
    retSale05M091 = dbSale.queryFromPool(stringSQL);
    ...
    for (int  i=0  ;  i<tb1_string.length  ;  i++) {
       ...
       //
       stringSQL = " UPDATE Sale05M091 SET " + 
                         "  StatusCd = 'C', " +
                         "  TrxDate = '"+ stringTrxDate +  "' " +
                    " WHERE OrderNo  =  '" +  stringOrderNo            +  "' "  + 
                    " and  CustomNo  =  '"   +  stringCustomNo          +  "' "  +
                    " and  ISNULL(StatusCd,'')='' ";                                                                           
      ...
      stringSQL = " INSERT  " + 
                    ...
                    " VALUES " +
                      ...
                      if (tb2_string.length<tb1_string.length){
                       ...
                       stringSQL                      = stringSQL+   " N'" + stringCustomNoNew               +  "' , " +
                                                       " N'" + stringCustomNewName          +  "' , " +
                                                       " '', ";
    ...
    for (int  i=0  ;  i<tb2_string.length  ;  i++) {
      ...
      if ("1".equals(stringChangeName)){
          stringSQL = " INSERT  INTO Sale05M091  ( " +
                    "                             OrderNo,RecordNo,auditorship,Nationality,CountryName, "+
                    "                             CustomNo,CustomName,Percentage,Birthday,MajorName, "+
                    "                             PositionName,ZIP,City,Town,Address,Cellphone, "+
                    "                             Tel,Tel2,eMail,IsBlackList,IsLinked,IsControlList,TrxDateDown "+
                    " ) VALUES ( " +
                                 "'"+ stringOrderNo+ "',"+
                                 "N'" +RecordNo+ "'," +
                                 "N'" +stringAuditorship+ "'," +
    ...
    // stringSQL=  "DELETE FROM   Sale05M091Ben WHERE OrderNo = '" + getValue("OrderNo") + "'" ;
    ...
    if(tb5_string.length>0){
      ...
      for (int  j=0  ;  j<tb5_string.length  ;  j++) {
        ...
        stringSQL = " INSERT INTO Sale05M091Ben (OrderNo,RecordNo,CustomNo,BenName,BCustomNo,Birthday,CountryName,HoldType,IsBlackList,IsControlList,IsLinked ) " +
                  " VALUES   ( " +
                            "'" + getValue("OrderNo")           + "',"+                        
                            "N'" + recordNo + "'," +
                            "N'" +stringCustomNo        + "'," +
                            ...
                            "N'" +str91BenNo          + "'," +
                            "N'" +str91BenBirthday      + "'," +
                            "N'" +str91BenCountryName  + "'," +
                            "N'" +str91BenHoldType    + "'," +
                            "N'" +str91BenIsBlackList     + "'," +
                            "N'" +str91BenIsControlList + "'," +
                            "N'" +str91BenIsLinked      + "' " +
                  " ) ";
    ...
    //20200226 kyle : 要求改成像客戶換名那樣保留資料並用StatusCd='C'標記
    //stringSQL=  "DELETE FROM   Sale05M091Agent WHERE OrderNo = '" + getValue("OrderNo") + "'" ;
      ...
      for (int  k=0  ;  k<tb6_string.length  ;  k++) {
        ...
        stringSQL = " INSERT INTO Sale05M091Agent (OrderNo,RecordNo,CustomNo,AgentName,ACustomNo,CountryName,AgentRel,AgentReason,IsBlackList,IsControlList,IsLinked ) " +
                  " VALUES   (    " +   
                            ...
                            "N'" +str91AgentName        + "'," +
                            "N'" +str91ACustomNo        + "'," +
                            "N'" +str91AgentCountryName + "'," +
                            "N'" +str91AgentRel         + "'," +
                            "N'" +str91AgentReason      + "'," +
                            "N'" +str91AgentIsBlackList     + "'," +
                            "N'" +str91AgentIsControlList   + "'," +
                            "N'" +str91AgentIsLinked      + "' " +
                          " ) ";
    ...
    if(vectorSql.size()  >  0) {
      ...
      //getButton("button4").setVisible(true);  //20200212 kyle mod: 刪除會造成實質受益人偏移，估計USER用不到這個功能先隱藏
    }
    //
    stringSQL=  "SELECT * FROM Sale05M091 "+
               " WHERE OrderNo = '" + getValue("OrderNo") + "'" +
               " and isnull(StatusCd,'') = '' "  ;
    ...
    stringSQL=  "Delete from  Sale05M093  "+
               " WHERE OrderNo = '" + getValue("OrderNo") + "'" +
               " and CustomNo = '"    + stringCustomNo           + "' ";
    dbSale.execFromPool(stringSQL); 
    ...
    getButton("btnRiskCheck").doClick();
