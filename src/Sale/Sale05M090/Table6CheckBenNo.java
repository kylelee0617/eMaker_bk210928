  ...
  public boolean check(String value)throws Throwable{
    ...
    if(!"".equals(value)){
      ...
      if(retQuery.length  >  0) { 
          setValueAt("table6", retQuery[0][0].trim(), s_row,"BenName");
          setValueAt("table6", retQuery[0][1].trim(), s_row,"IsBlackList");
          setValueAt("table6", retQuery[0][2].trim(), s_row,"IsControlList");
          setValueAt("table6", retQuery[0][3].trim(), s_row,"IsLinked");
          ...
          if(retCList.length > 0) {
            if("".equals(errMsg)){
              errMsg ="實質受益人"+retQuery[0][0].trim()+"為控管之制裁名單對象，請禁止交易，並依洗錢防制內部通報作業送呈法遵室。";
            }else{
              errMsg =errMsg+"\n實質受益人"+retQuery[0][0].trim()+"為控管之制裁名單對象，請禁止交易，並依洗錢防制內部通報作業送呈法遵室。";
            }
          }
          //制裁名單171PEPS
          ...
          if(ret171List.length > 0) {
            if("".equals(errMsg)){
              errMsg ="實質受益人"+retQuery[0][0].trim()+"、家庭成員及有密切關係之人，為重要政治性職務人士，請加強客戶盡職調查，並依洗錢及資恐防制作業辦理。";
            }else{
              errMsg =errMsg+"\n實質受益人"+retQuery[0][0].trim()+"、家庭成員及有密切關係之人，為重要政治性職務人士，請加強客戶盡職調查，並依洗錢及資恐防制作業辦理。";
            }
          }
          
          /*
          // 萊斯Start
          String birth = getValueAt("table6", s_row,  "Birthday").toString().length()==0? " ":getValueAt("table1", s_row,  "Birthday").toString();
          String Ind = " ";
          ...
          */
          ...
          if("Y".equals(retQuery[0][1].trim()) || "Y".equals(retQuery[0][2].trim())){
            if("".equals(errMsg)){
              errMsg ="實質受益人"+retQuery[0][0].trim()+"為疑似黑名單對象，請覆核確認後，再進行後續交易相關作業。";
            }else{
              errMsg =errMsg+"\n實質受益人"+retQuery[0][0].trim()+"為疑似黑名單對象，請覆核確認後，再進行後續交易相關作業。";
            }
          }
          if("Y".equals(retQuery[0][3].trim())){//利關人
            if("".equals(errMsg)){
              errMsg ="實質受益人"+retQuery[0][0].trim()+"為公司利害關系人，請依保險業與利害關係人從事放款以外之其他交易管理辦法執行。";
            }else{
              errMsg =errMsg+"\n實質受益人"+retQuery[0][0].trim()+"為公司利害關系人，請依保險業與利害關係人從事放款以外之其他交易管理辦法執行。";
            }
          }
          if(!"".equals(errMsg)){
            messagebox(errMsg);
          ...
          setValueAt("table6","", s_row,"BenName");
