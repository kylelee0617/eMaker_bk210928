...
public class Sale05R27401_New extends bTransaction{
    ...
    public void doExcel() throws Throwable{
        ...
        for(int i = 0; i < retCust.length; i++) {
          ...
          stringSql="SELECT " + 
                "c.OrderNo,c.RecordNo,c.auditorship,c.Nationality,c.CountryName,c.CustomNo,c.CustomName, " +                
                "c.IsLinked,c.IsControlList,c.IsBlackList,c.TrxDate,c.TitleCD, c.RiskValue  " + //8 9
                "FROM SALE05M091 c Inner Join SALE05M275_new n On c.OrderNo = n.OrderNo " +
                  "WHERE ISNULL(c.StatusCd , '') = '' AND n.ContractNo = '" + stringContractNo + "' And c.CustomNo = '" + retCust[i][0] + "'" ;
          ...
          for(int j = 0; j<retCustom.length; j++) {
            ...
            if(retCustom[j][5].length() == 8){
              ...
              String sql91Ben = "SELECT c.IsControlList,c.IsBlackList,c.BCustomNo FROM SALE05M091ben c Inner Join SALE05M275_new n On c.OrderNo = n.OrderNo "
                  + "WHERE n.ContractNo = '"+stringContractNo+"'  AND c.CustomNo = '"+retCustom[j][5]+"' AND ISNULL(c.StatusCd , '') = '' ";
...
//                        "AND ISREMOVE = 'N' " +
                    ...
                    for(int intNo=0; intNo<retData.length; intNo++){
                        ...
                        if(retSales.length > 0){
                            if(stringLimitIDCS.indexOf(retData[intNo][3]) == -1){
                                stringLimitIDCS = stringLimitIDCS.length()==0?
                                                retData[intNo][1]:
                                                stringLimitIDCS+","+retData[intNo][1];
                            }
                        }
                    }
                }else{
                    // 先暫存限定Sale05M315人員ID
                    if("H56A".equals(stringProjectID1) || "H73A".equals(stringProjectID1) || "H85A".equals(stringProjectID1) || "H93A".equals(stringProjectID1) || "H106A".equals(stringProjectID1)){
                        stringSql = "SELECT (CASE WHEN SUBSTRING(SalesNo,1,1)='C' THEN 'C0'+RIGHT(SalesNo,3) "+
                                                 "ELSE 'B'+SalesNo END) SalesNo "+
                                  "FROM Sale05M315 "+
                        ...
                        for(int intRow=0; intRow<retData.length; intRow++){
                            stringLimitIDCS = stringLimitIDCS.length()==0?
                                            retData[intRow][0]:
                                            stringLimitIDCS+"," +retData[intRow][0];
                        }
                    ...
                    for(int intRow=0; intRow<retData.length; intRow++){
                        ...
                        if(retSales.length > 0){
                            if(stringSalesCS.indexOf(retData[intRow][3]) == -1){
                                ...
                                stringLimitIDCS = stringLimitIDCS.length()==0?
                                                retData[intRow][4]:
                                                stringLimitIDCS+","+retData[intRow][4];
                            }
                            if(blnHasCS){
                                stringSql = "INSERT INTO Sale05M295Print(ContractNo, "+
                                                                     "CompanyCd, "+
        ...
        for(int intRow=0; intRow<aryHistoryData.length; intRow++){
            if((("N".equals(stringStatus) || "NA".equals(stringStatus) || "A".equals(stringStatus)) && "承辦".equals(aryHistoryData[intRow][0]))  ||  "財務代銷".equals(aryHistoryData[intRow][0])){ // 審核人
                if(stringSignerandTime1.length() == 0){
                   ...
                   "內業業管".equals(aryHistoryData[intRow][0])  ||  "行銷".equals(aryHistoryData[intRow][0])  ||  
                   "營業經辦".equals(aryHistoryData[intRow][0])  ||  "營業部".equals(aryHistoryData[intRow][0])){
        ...
        for(int intRow=0; intRow<aryHistoryData.length; intRow++){
            ...
            }else if("內業業管".equals(aryHistoryData[intRow][0])  ||  "行銷".equals(aryHistoryData[intRow][0])  ||  
                   "營業經辦".equals(aryHistoryData[intRow][0])  ||  "營業部".equals(aryHistoryData[intRow][0])){
        ...
        if(retData.length > 0){
            ...
            if("CS".equals(retData[retData.length-1][0])){
                ...
                if(blnIsEndState){
                    // 審核人(內業業管、行銷、財務代銷、營業經辦、營業部、開發經辦、開發部、財務經辦、財務室、法務室)、不動產、會計
                    //exeExcel.putDataIntoExcel(  6,  intInsertSaleRow2,  stringIDandTimeCS1,  objectSheet1); //不動產審核
                    //exeExcel.putDataIntoExcel(  4,  intInsertSaleRow2,  stringIDandTimeCS2,  objectSheet1); // 不動產部
