  ...
  public String getDefaultValue(String value) throws Throwable {
    ...
    if (stringFlowStatus.equals("業管-審核")) {
      ...
      for (int intSale05M084 = 0; intSale05M084 < retSale05M084P.length; intSale05M084++) {
        ...
        if ("2".equals(stringNationality))
          stringInvoiceKind = "2";
        if ("4".equals(stringNationality))
          stringInvoiceKind = "2"; // 20100401 新增
...
// 由 Sale05M084 取得之所有客戶 START
      ...
      for (int intSale05M084P = 0; intSale05M084P < retSale05M084P.length; intSale05M084P++) {
        ...
        if ("2".equals(stringNationality))
          stringInvoiceKind = "2";
        if ("3".equals(stringNationality))
          stringInvoiceKind = "2"; // 2010-3-5 增加
// 取得特定客戶的購屋證明單 START                           
        ...
        for (int intSale05M086 = 0; intSale05M086 < retSale05M086.length; intSale05M086++) {
          ...
          for (int intSale05M081 = 0; intSale05M081 < retSale05M081.length; intSale05M081++) {
            ...
            if ("01002".equals(stringITEMLS_CD)) {
              if (doParseDouble(stringH_InvoiceMoney) > 0)
                hashtablePosition.put(stringPosition + "_H", "Y");
              if (doParseDouble(stringL_InvoiceMoney) > 0)
                hashtablePosition.put(stringPosition + "_L", "Y");
            }
            // 1 建物款 2 土地款
            for (int intInvoice = 1; intInvoice <= 2; intInvoice++) {
              ...
              // 房屋款
              if (intInvoice == 1)
                if (stringH_InvoiceMoney.length() == 0 || doParseFloat(stringH_InvoiceMoney) == 0)
                  continue;
              // 土地款
              if (intInvoice == 2)
                if (stringL_InvoiceMoney.length() == 0 || doParseFloat(stringL_InvoiceMoney) == 0)
                  continue;
              // 房屋-建物款
            ...
            for (int intInvoice = 1; intInvoice <= 2; intInvoice++) {
              ...
              if (doParseDouble(stringFeeMoney) <= 0)
                continue;
              // 房屋-建物款
              if (stringHouseCar.equals("House") && intInvoice == 1) {
                intType = 4;
...
// 由 Sale05M084 取得之所有客戶  END
      ...
      for (int intSale05M084P = 0; intSale05M084P < retSale05M084P.length; intSale05M084P++) {
        ...
        if ("2".equals(stringNationality))
          stringInvoiceKind = "2";
        if ("3".equals(stringNationality))
          stringInvoiceKind = "2"; // 2101-3-5
        if ("4".equals(stringNationality))
          stringInvoiceKind = "2"; // 20100401
        for (int intPositionNo = 0; intPositionNo < vectorUniquePosition.size(); intPositionNo++) {
          ...
          for (int intCompanNo = 0; intCompanNo < vectorCompanyCd.size(); intCompanNo++) {
            ...
            for (int intNo = 0; intNo < 4; intNo++) {
              ...
              for (int intRecord = 1; intRecord <= intRecordMax; intRecord++) {
                ...
                /* 發票.InvoM031(Body)(多筆) */
                if (doParseInteger(stringORDER_NO) <= 0)
                  stringORDER_NO = "";
      ...
      if (vectorSql.size() > 0)
        dbInvoice.execFromPool((String[]) vectorSql.toArray(new String[0]));
      //
  ...
  public void doInvoM022Undo(Vector vectorCompanyCd, int intCompanySize) throws Throwable {
    ...
    String[][] retSale05M085UPDATE = null;
    //
    if (intCompanySize >= vectorCompanyCd.size())
      return;
  ...
  public String doStringSubstring(String stringObject, int intStart, int intEnd) {
    String stringRet = "";
    // intStart
    if (stringObject.length() < intStart)
      return stringRet;
  ...
  public String getInvoiceTotalMoney(String stringInvoiceNo) throws Throwable {
    ...
    if (retInvoM030.length > 0)
      stringInvoiceTotalMoney = retInvoM030[0][0].trim();
  ...
  // 寫入客戶檔
//    public  void  doInvertInvoM0C0_new(String  stringCustomNo,  String  stringORDER_NO) throws  Throwable {
//      //        
//      String  stringSql                 =  "" ;
