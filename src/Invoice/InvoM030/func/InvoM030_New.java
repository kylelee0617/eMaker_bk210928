...
/**
 * Tip:
 * 硂娩秨琌︽綪祇布( ΙProcessInvoiceNo=1 )
 * 
 * @author B04391
 *
 */
...
//    dbInvoice = getTalk("Invoice");
    ...
    */  
    ...
    for(int i=0;i<retInvoM022.length;i++){    //碞top 1 禲を癹伴临琌谋眔top 1 ぃ穦Τ掸?
...
//    System.out.println("眔祇布腹絏>>>" + stringNowInvoiceNo);
    if (stringNowInvoiceNo.length() < 10){
      message("筿福祇布ノЧ 叫癩叭烩!");
      return false;
    }
    
    message("眔祇布腹絏>>>" + stringNowInvoiceNo);
    ...
    try {
      ...
      sbSQL.append("'").append(invoiceTime).append("', ");                        //20201210 Kyle 糤祇布丁
      ...
      sbSQL.append("").append(getValue("InvoiceMoney").trim()).append(", ");      //ゼ祙
      sbSQL.append("").append(getValue("InvoiceTax").trim()).append(", ");        //祙肂
      sbSQL.append("").append(getValue("InvoiceTotalMoney").trim()).append(", "); //祙
      sbSQL.append("'").append(getValue("TaxKind").trim()).append("', ");         //祙
      sbSQL.append("").append(0).append(", ");                                    //ч琵肂
      sbSQL.append("").append(0).append(", ");                                    //ч琵Ω计
      sbSQL.append("'").append("N").append("', ");                                //YN
      sbSQL.append("").append(0).append(", ");                                    //干Ω计
      sbSQL.append("'").append("N").append("', ");                                //紀YN
      sbSQL.append("'").append("N").append("', ");                                //眀YN
      sbSQL.append("'").append("1").append("', ");                                //ProcessInvoiceNo
      sbSQL.append("'").append("祇布╰参").append("', ");                                      //Transfer
      ...
      sbSQL.append("'").append(customName).append("' ");                         //め﹎
      sbSQL.append(") ");
      ...
      sbSQL.append("values ");
      sbSQL.append("(");
      sbSQL.append("'").append(stringNowInvoiceNo).append("', ");                 //祇布腹絏
      sbSQL.append("'").append(getValue("InvoiceDate").trim()).append("', ");     //祇布ら戳
      sbSQL.append("'").append(getValue("InvoiceKind").trim()).append("', ");     //祇布羛Α
      sbSQL.append("'").append(getValue("CompanyNo").trim()).append("', ");       //そ絏
      sbSQL.append("'").append(getValue("DepartNo").trim()).append("', ");        //场絏
      sbSQL.append("'").append(getValue("ProjectNo").trim()).append("', ");       //絏
      sbSQL.append("'").append(getValue("InvoiceWay").trim()).append("', ");      //Invoice Way
      sbSQL.append("'").append(getValue("HuBei").trim()).append("', ");           //め腹
      sbSQL.append("'").append(getValue("CustomNo").trim()).append("', ");        //め腹
      sbSQL.append("'").append(getValue("PointNo").trim()).append("', ");         //篕璶
      sbSQL.append("").append(getValue("InvoiceMoney").trim()).append(", ");      //ゼ祙
      sbSQL.append("").append(getValue("InvoiceTax").trim()).append(", ");        //祙肂
      sbSQL.append("").append(getValue("InvoiceTotalMoney").trim()).append(", "); //祙
      sbSQL.append("'").append(getValue("TaxKind").trim()).append("', ");         //祙
      ...
      sbSQL.append("'").append("N").append("', ");         //紀YN
      sbSQL.append("'").append("N").append("', ");         //眀YN
      sbSQL.append("'").append("").append("', ");          //祇布矪瞶よΑ
      sbSQL.append("'").append("Μ蹿").append("' ");       //Μ蹿/狝
      sbSQL.append(") ");
...
//      as400.execFromPool( (String[]) vectorSql.toArray(new String[0]) );
      ...
      message("玻ネ祇布 = " +  stringNowInvoiceNo);   
    } catch(Exception ex) {
      message("祇ネ岿粇1:" + ex);
      return false;
    }
