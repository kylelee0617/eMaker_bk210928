...
public class FuncPrint_bk_BeforeEMK20210115004 extends bTransaction{
    ...
    // 檢核
      ...
      public boolean isBatchCheckOK() throws Throwable{
        ...
        if(stringStartDate.length() == 0){
            ...
            if(stringStartDate.replaceAll("/","").length() != 8){
                message("[日期-起] 日期格式錯誤(YYYY/MM/DD)。");
                getcLabel("StartDate").requestFocus();
                return false;
            }
            stringStartDate = exeFun.getDateAC(stringStartDate, "日期-起");
            if(stringStartDate.length() != 10){
                message(stringStartDate);
                getcLabel("StartDate").requestFocus();
                return false;
            }else{
                setValue("StartDate", stringStartDate);
            }
        }
        // 日期-迄
        ...
        if(stringEndDate.length() == 0){
            ...
            if(stringEndDate.replaceAll("/","").length() != 8){
                message("[日期-迄] 日期格式錯誤(YYYY/MM/DD)。");
                getcLabel("EndDate").requestFocus();
                return false;
            }
            stringEndDate = exeFun.getDateAC(stringEndDate, "日期-迄");
            if(stringEndDate.length() != 10){
                message(stringEndDate);
                getcLabel("EndDate").requestFocus();
                return false;
            }else{
                setValue("EndDate", stringEndDate);
            }
        }
        //
        if(stringStartDate.length() > 0  &&  stringEndDate.length() > 0){
            if(datetime.subDays1(stringEndDate.replaceAll("/",""),stringStartDate.replaceAll("/","")) < 0){
                message("[日期] 日期起迄錯誤!");
                return false;
            }
        }
    ...
    public void doExcelNew() throws Throwable{
        ...
        /*
          接待中心      701/702/703/704/710/730
          POP           781/782/783/784/785/786
          報紙            740/741/743/744/745/746/748/749
          雜誌            750
          電視            771
          廣播            772
          電影院       773
        ...
        */
        ...
        // 排除案別
        stringSql = "SELECT RTRIM(ProjectID) "+
                  "FROM AO_SPSale03R530B_DenyProjectID";
        ...
        stringSql = "SELECT DISTINCT RTRIM(ProjectID1) "+
                  "FROM A_Sale "+
                  "WHERE OrderDate BETWEEN '"+stringStartDate+"' AND '"+stringEndDate+"' "+
                  "AND DealMoney>0 "+
        ...
        // 來人
        stringSql = "SELECT DISTINCT RTRIM(CASE WHEN Series_Name='NEWH82A' THEN 'H82A' ELSE Series_Name END) "+
                  "FROM MIS_NewAOMixView WITH (NOLOCK) "+
                  "WHERE CONVERT(char(10),Create_Time,111) BETWEEN '"+stringStartDate+"' AND '"+stringEndDate+"' "+
                  "AND (Nego_Type IN ('新來人','複來訪') OR Nego_Way='來電') "+
                  "AND ISNULL(value_point,0)>0 "+
                  stringNotInProjectID1Sql2;
        retData = dbAO.queryFromPool(stringSql);
        for(int intRow=0; intRow<retData.length; intRow++){
        ...
        stringSql = "SELECT DISTINCT RTRIM(ProjectID1) "+
                  ...
                  "AND DepartNo<>'0531' "+
                  "AND CostID1<>'' "+
                  "AND UseDate BETWEEN '"+stringStartDate+"' AND '"+stringEndDate+"' "+
                  stringNotInProjectID1Sql;
        ...
        String[] staticProjectList = {"H92A","H93A","H95A","H98A","H99A","H101A","H102A","H103A","H106A","H100A","H110A","H111A"};
        ...
        stringSql = "SELECT DISTINCT RTRIM(ProjectID1) "+
                  ...
                  "AND UseDate BETWEEN '"+stringStartDate+"' AND '"+stringEndDate+"' "+
                  stringNotInProjectID1Sql;
        retData = dbDocCS.queryFromPool(stringSql);
        for(int intRow=0; intRow<retData.length; intRow++){
            if(vcTemp.indexOf(retData[intRow][0]) == -1){
                vcTemp.addElement(retData[intRow][0]);
            }
        ...
        */
        ...
        //
        // 日期
        exeExcel.putDataIntoExcel(  0,  0,  "日期："+stringStartDate+"~"+stringEndDate,  objectSheet1);
        //20180925 轉換 value_point = null 的資料
        String getValueNullPointSql = "SELECT Small_ID, Value_Point, count(*) FROM Cus_SmallMixView WHERE Value_Point is Null And Create_Time between '"+stringStartDate+" 00:00:00' and '"+stringEndDate+" 23:59:59' GROUP BY Small_ID, Value_Point, Create_Time ORDER BY Create_Time";
            ...
            for(int intRow=0; intRow<aryProjectID1.length; intRow++){
                ...
                if(stringSaleType.equals("銷售")){
                    stringSql = "SELECT ISNULL(SUM(DealMoney),0) "+
                              "FROM A_Sale "+
                              "WHERE ProjectID1='"+stringProjectID1+"' "+
                              "AND OrderDate BETWEEN '"+stringStartDate+"' AND '"+stringEndDate+"' "+
                              "AND (LEN(Position) > 0 OR LEN(Car) > 0) " +                  
                              "AND L_Com = '8'";                             
                              //"AND (ComNo='06' OR ComNo='20' OR ComNo IS NULL OR LEN(ComNo)=0)";                
                }else{
                    stringSql = "SELECT ISNULL(SUM(OKSale),0) "+
                              "FROM Sale02M050 " + 
                              "WHERE ProjectID1='"+stringProjectID1+"' "+
                ...
                intInsertDataRow = 12;
                // 分類
                stringSql = "SELECT ISNULL(SUM(CASE WHEN Nego_Type='新來人' THEN value_point ELSE 0 END),0),"+
                          "ISNULL(SUM(CASE WHEN Nego_Type IN ('新來人','複來訪') THEN value_point ELSE 0 END),0),"+
                          "ISNULL(SUM(CASE WHEN Nego_Way='來電' THEN value_point ELSE 0 END),0) "+
                          "FROM MIS_NewAOMixView WITH (NOLOCK) "+
                          //"WHERE (CASE WHEN Series_Name='NEWH82A' THEN 'H82A' ELSE Series_Name END)='"+stringProjectID1+"' "+
                          stringAOProjectID1Sql+
                          "AND CONVERT(char(10),Create_Time,111) BETWEEN '"+stringStartDate+"' AND '"+stringEndDate+"'";
                retData   = dbAO.queryFromPool(stringSql);
                exeExcel.putDataIntoExcel(intInsertDataCol,      intInsertDataRow,      retData[0][0],  objectSheet1);
                ...
                stringSql = "SELECT ISNULL(ROUND(SUM(CASE WHEN NewCode2='C01' THEN value_point ELSE 0 END),1),0) MediaSum1, "+
                          "ISNULL(ROUND(SUM(CASE WHEN NewCode1='K' THEN value_point ELSE 0 END),1),0) MediaSum2, "+
                          "ISNULL(ROUND(SUM(CASE WHEN NewCode2='I04' THEN value_point ELSE 0 END),1),0) MediaSum3, "+
                          "ISNULL(ROUND(SUM(CASE WHEN NewCode2='I05' THEN value_point ELSE 0 END),1),0) MediaSum4, "+
                          "ISNULL(ROUND(SUM(CASE WHEN NewCode2='I06' THEN value_point ELSE 0 END),1),0) MediaSum5, "+
                          "ISNULL(ROUND(SUM(CASE WHEN NewCode2='I07' THEN value_point ELSE 0 END),1),0) MediaSum6, "+
                          "ISNULL(ROUND(SUM(CASE WHEN NewCode2='I08' THEN value_point ELSE 0 END),1),0) MediaSum7, "+
                          ...
                          "ISNULL(ROUND(SUM(CASE WHEN NewCode4='I120101' THEN value_point ELSE 0 END),1),0) MediaSum10, "+
                          "0 MediaSum11, "+
                          "ISNULL(ROUND(SUM(CASE WHEN NewCode1='D' THEN value_point ELSE 0 END),1),0) MediaSum12, "+
                          "ISNULL(ROUND(SUM(CASE WHEN NewCode1='L' THEN value_point ELSE 0 END),1),0) MediaSum13 "+
                          "FROM MIS_NewAOMixView WITH (NOLOCK) "+
                          //"WHERE (CASE WHEN Series_Name='NEWH82A' THEN 'H82A' ELSE Series_Name END)='"+stringProjectID1+"' "+
                          stringAOProjectID1Sql+
                          "AND CONVERT(char(10),Create_Time,111) BETWEEN '"+stringStartDate+"' AND '"+stringEndDate+"' "+
                ...
                // 複來訪
                intInsertDataRow = 28;
                stringSql = "SELECT SUM(value_point) "+
                          "FROM MIS_NewAOMixView WITH (NOLOCK) "+
                          //"WHERE (CASE WHEN Series_Name='NEWH82A' THEN 'H82A' ELSE Series_Name END)='"+stringProjectID1+"' "+
                          stringAOProjectID1Sql+
                          "AND CONVERT(char(10),Create_Time,111) BETWEEN '"+stringStartDate+"' AND '"+stringEndDate+"' "+
                ...
                intInsertDataRow = 46;
                if(stringSaleType.equals("銷售")){
                    stringSql = "SELECT ISNULL(SUM(CASE WHEN SUBSTRING(SSMediaID,1,3)='C01' THEN DealMoney ELSE 0 END),0), "+
                              "ISNULL(SUM(CASE WHEN SUBSTRING(SSMediaID,1,1)='K' THEN DealMoney ELSE 0 END),0),"+
                              "ISNULL(SUM(CASE WHEN SUBSTRING(SSMediaID,1,3)='I04' THEN DealMoney ELSE 0 END),0),"+
                              "iSNULL(SUM(CASE WHEN SUBSTRING(SSMediaID,1,3)='I05' THEN DealMoney ELSE 0 END),0),"+
                              "ISNULL(SUM(CASE WHEN SUBSTRING(SSMediaID,1,3)='I06' THEN DealMoney ELSE 0 END),0),"+
                              "ISNULL(SUM(CASE WHEN SUBSTRING(SSMediaID,1,3)='I07' THEN DealMoney ELSE 0 END),0),"+
                              "ISNULL(SUM(CASE WHEN SUBSTRING(SSMediaID,1,3)='I08' THEN DealMoney ELSE 0 END),0),"+
                              "ISNULL(SUM(CASE WHEN SUBSTRING(SSMediaID,1,3)='I09' THEN DealMoney ELSE 0 END),0),"+
                              "ISNULL(SUM(CASE WHEN SUBSTRING(SSMediaID,1,3) IN ('I10','I11','I12') THEN DealMoney ELSE 0 END),0),"+
                              "ISNULL(SUM(CASE WHEN SSMediaID='I120101' THEN DealMoney ELSE 0 END),0),"+
                              "0, "+
                              "ISNULL(SUM(CASE WHEN SUBSTRING(SSMediaID,1,1)='D' THEN DealMoney ELSE 0 END),0),"+
                              "ISNULL(SUM(CASE WHEN SUBSTRING(SSMediaID,1,1)='L' THEN DealMoney ELSE 0 END),0) "+
                              "FROM A_Sale "+
                              "WHERE ProjectID1='"+stringProjectID1+"' "+
                              "AND OrderDate BETWEEN '"+stringStartDate+"' AND '"+stringEndDate+"' "+
                              "AND (LEN(Position) > 0 OR LEN(Car) > 0) " +                  
                    ...
                    stringSql = "SELECT ISNULL(SUM(CASE WHEN SUBSTRING(SSMediaID,1,3)='C01' THEN OKSale ELSE 0 END),0), "+
                              "ISNULL(SUM(CASE WHEN SUBSTRING(SSMediaID,1,1)='K' THEN OKSale ELSE 0 END),0),"+
                              "ISNULL(SUM(CASE WHEN SUBSTRING(SSMediaID,1,3)='I04' THEN OKSale ELSE 0 END),0),"+
                              "iSNULL(SUM(CASE WHEN SUBSTRING(SSMediaID,1,3)='I05' THEN OKSale ELSE 0 END),0),"+
                              "ISNULL(SUM(CASE WHEN SUBSTRING(SSMediaID,1,3)='I06' THEN OKSale ELSE 0 END),0),"+
                              "ISNULL(SUM(CASE WHEN SUBSTRING(SSMediaID,1,3)='I07' THEN OKSale ELSE 0 END),0),"+
                              "ISNULL(SUM(CASE WHEN SUBSTRING(SSMediaID,1,3)='I08' THEN OKSale ELSE 0 END),0),"+
                              "ISNULL(SUM(CASE WHEN SUBSTRING(SSMediaID,1,3)='I09' THEN OKSale ELSE 0 END),0),"+
                              "ISNULL(SUM(CASE WHEN SUBSTRING(SSMediaID,1,3) IN ('I10','I11','I12') THEN OKSale ELSE 0 END),0),"+
                              "ISNULL(SUM(CASE WHEN SSMediaID='I120101' THEN OKSale ELSE 0 END),0),"+
                              "0, "+
                              "ISNULL(SUM(CASE WHEN SUBSTRING(SSMediaID,1,1)='D' THEN OKSale ELSE 0 END),0),"+
                              "ISNULL(SUM(CASE WHEN SUBSTRING(SSMediaID,1,1)='L' THEN OKSale ELSE 0 END),0) "+
                              "FROM Sale02M050 " + 
                              "WHERE ProjectID1='"+stringProjectID1+"' "+
                              "AND BuyerDate BETWEEN '"+stringStartDate+"' AND '"+stringEndDate+"'";
                }
                ...
                intInsertDataRow = 61;
                if(stringSaleType.equals("銷售")){
                    stringSql = "SELECT ISNULL(SUM(CASE WHEN SUBSTRING(SSMediaID,1,3)='C01' THEN 1 ELSE 0 END),0), "+
                              "ISNULL(SUM(CASE WHEN SUBSTRING(SSMediaID,1,1)='K' THEN 1 ELSE 0 END),0),"+
                              "ISNULL(SUM(CASE WHEN SUBSTRING(SSMediaID,1,3)='I04' THEN 1 ELSE 0 END),0),"+
                              "iSNULL(SUM(CASE WHEN SUBSTRING(SSMediaID,1,3)='I05' THEN 1 ELSE 0 END),0),"+
                              "ISNULL(SUM(CASE WHEN SUBSTRING(SSMediaID,1,3)='I06' THEN 1 ELSE 0 END),0),"+
                              "ISNULL(SUM(CASE WHEN SUBSTRING(SSMediaID,1,3)='I07' THEN 1 ELSE 0 END),0),"+
                              "ISNULL(SUM(CASE WHEN SUBSTRING(SSMediaID,1,3)='I08' THEN 1 ELSE 0 END),0),"+
                              "ISNULL(SUM(CASE WHEN SUBSTRING(SSMediaID,1,3)='I09' THEN 1 ELSE 0 END),0),"+
                              "ISNULL(SUM(CASE WHEN SUBSTRING(SSMediaID,1,3) IN ('I10','I11','I12') THEN 1 ELSE 0 END),0),"+
                              "ISNULL(SUM(CASE WHEN SSMediaID='I120101' THEN 1 ELSE 0 END),0),"+
                              "0, "+
                              "ISNULL(SUM(CASE WHEN SUBSTRING(SSMediaID,1,1)='D' THEN 1 ELSE 0 END),0),"+
                              "ISNULL(SUM(CASE WHEN SUBSTRING(SSMediaID,1,1)='L' THEN 1 ELSE 0 END),0) "+
                              "FROM A_Sale "+
                              "WHERE ProjectID1='"+stringProjectID1+"' "+
                              "AND OrderDate BETWEEN '"+stringStartDate+"' AND '"+stringEndDate+"' "+
                              "AND L_Com = '8'" +
                ...
                for(int  intRowL=67  ;  intRowL<94  ;  intRowL++) {
                    stringCostID  =  exeExcel.getDataFromExcel2(5,   intRowL,  objectSheet1) ;      if("".equals(stringCostID))  continue ;
                    arrayCostID  =  convert.StringToken(stringCostID,  "\n") ;
                    doubleTemp  =  0 ;
                    for(int  intL=0  ;  intL<arrayCostID.length  ;  intL++) {
                          stringCostID  =  arrayCostID[intL] ;    if("".equals(stringCostID))  continue ;
                ...
                /*arrayMoney = (String[])hashtableMoney.get(stringProjectID1);
                if(arrayMoney == null){
                    arrayMoney = new String[arrayCostID.length];
                    for(int intNoL=0; intNoL<arrayMoney.length; intNoL++) arrayMoney[intNoL] = "0";
                }
                for(int x=0; x<arrayMoney.length-1; x++){
                    System.out.println("Col("+intInsertDataCol+")Row("+(intInsertDataRow+1)+")Data("+arrayMoney[x]+")-------------------------------");
                    exeExcel.putDataIntoExcel(intInsertDataCol,  intInsertDataRow++,  arrayMoney[x],  objectSheet1);
                }*/
                ...
                intInsertDataRow = 93;
                /*
                stringSql = "SELECT SUM(RealMoney) RealMoney "+
                          "FROM Z_CoReaMM "+
                          "WHERE ProjectID1='"+stringProjectID1+"' "+
                          "AND YYMM BETWEEN '"+stringStartDate+"' AND '"+stringEndDate+"' "+
                          "AND (ComNo='06' OR ComNo='20' OR ComNo IS NULL OR LEN(ComNo)=0) ";
                retData = dbSale.queryFromPool(stringSql);
                exeExcel.putDataIntoExcel(intInsertDataCol,  intInsertDataRow,  retData[0][0],  objectSheet1);
                */
    ...
    public  void  doCetMoney(String  stringType,  String  stringDateStart,  String  stringDateEnd,  String  stringProjectID1AndSql,  FargloryUtil  exeUtil,  talk  dbDoc,  talk  dbDocCS,  Hashtable  hashtableMoney) throws  Throwable {
        String      stringSql                         =  "" ;
        ...
        if(dbDocCS  !=  null) {
            ...
            retData  =  dbDocCS.queryFromPool(stringSql.replaceAll("AND  ComNo  =  'CS'",  "")) ;
            doData(stringType,  retData,  exeUtil,  hashtableMoney) ;   
        }
        */
    }
    ...
    public  void  doData(String  stringType,  String[][]  retData,  FargloryUtil  exeUtil,  Hashtable  hashtableMoney) throws  Throwable {
        ...
        for(int  intNo=0  ;  intNo<retData.length  ;  intNo++) {
            ...
            if("B".equals(stringType)) {
                ...
                if("C".equals(stringSSMediaID)){stringCostID="70";stringCostID1="1";stringSSMediaID="";}  // C 接待中心(接待通路)
                if("K".equals(stringSSMediaID)){stringCostID="78";stringCostID1="1";stringSSMediaID="";}  // K POP通路
                if("I".equals(stringSSMediaID)) {stringCostID="89";stringCostID1="1";stringSSMediaID="";}   // I 媒體通路
                if("D".equals(stringSSMediaID)){stringCostID="86";stringCostID1="1";stringSSMediaID="";}  // D 網路通路
                if("L".equals(stringSSMediaID)){stringCostID="86";stringCostID1="3";stringSSMediaID="";}  // L 簡訊通路
            }
