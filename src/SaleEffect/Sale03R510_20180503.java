...
public class Sale03R510_20180503 extends bproc{
  public String getDefaultValue(String value)throws Throwable{
    ...
    public  Hashtable  doExcel1(int[] arrayRow, Dispatch  objectSheet1, FargloryExcel  exeExcel, FargloryUtil  exeUtil,  talk  dbAO,  talk  dbDoc,  talk  dbDocCS,  talk  dbSale)throws  Throwable {
        ...
        //String        stringFilePath                =  "g:\\資訊室\\Excel\\SaleEffect\\Sale03R510_2017-02-18.xlt" ;
        ...
        String[][]      retTitle2                     =  {{"直效通路",  "已購客戶",  "未購名單",  "異業合作",  "同業通路",  "企業團"},
                                                {"媒體通路",  "網路通路",  "簡訊"},
                                              {"POP",         "接待通路"}};
        ...
        retSale03R510A            =  dbSale.queryFromPool("speMakerSale03R510A  '"+stringProjectID1+"',  '"+stringYearAC+"',  '"+stringMonth+"' " )  ;    // 業績
        ...
        // 第二表格
        String[][]      retSale03R510B                =  dbSale.queryFromPool("speMakerSale03R510B  '"+stringProjectID1+"',  '"+stringYearAC+"',  '"+stringMonth+"' " )  ;    // 業績
        ...
        for(int  intRow=0  ;  intRow<=57  ;  intRow++) {
            ...
            if(hashtableData  !=  null) {
                for(int  intNo=0  ;  intNo<vectorTitles.size()  ;  intNo++) {
                    ...
                    stringValue   =  ""+hashtableData.get(stringTitle) ;              if("".equals(stringValue))  stringValue="0" ;if("null".equals(stringValue))  stringValue="0" ;
                    //
                ...
                // 遠雄之友
                stringValue   =  ""+hashtableData.get("遠雄之友") ;               if("".equals(stringValue)  ||  "null".equals(stringValue)) stringValue = "0" ;
                ...
                // PASS
                stringValue   =  ""+hashtableData.get("PASS") ;               if("".equals(stringValue)  ||  "null".equals(stringValue)) stringValue = "0" ;
                //System.out.println(stringRowLine+"業績("+getChangeExcelRow(intRow)+")("+stringValue+")-------------------------") ;
        ...
        if(retDataPosition  !=  null) {
            for(int  intNo=0  ;  intNo<retDataPosition.length  ;  intNo++) {
                ...
                if("null".equals(stringValue))  continue ;
                //
                //System.out.println("OTHER Col("+intCol+")Row("+intRowL+")值("+intRowL+")------------------------------------") ;
                exeExcel.putDataIntoExcel(intCol,  intRowL,  stringValue, objectSheet1) ;
            }
        }
    ...
    // 業績
    public  Hashtable  getSale03R510BH(String  stringProjectID1,  String  stringYearAC,  String  stringMonth,  String[][]  retSale03R510B,  FargloryUtil  exeUtil,  talk  dbSale)throws  Throwable {
        ...
        //                                2                         3                4
        //"列數",  "無",     "未購名單",     "直效通路",  "已購客戶",  "異業合作",  "同業通路",  "企業團",  "媒體通路",  "網路通路",  "簡訊",  "POP",  "接待通路",  "遠雄之友",  "PASS"
        //                                 3                        4                 2
        //                          "直效通路",     "已購客戶",  "未購名單"
        String  stringTemp2  =  "" ;
        String  stringTemp3  =  "" ;
    ...
    public  void  SetOrderList(String  stringTitile,  String  stringOrderType,  int  intGetDataRow,  int  intPutDataRow,  int  intColOrder11,  int  intColOrder12,  int  intRowOrder,  String[]  arraySSMediaID,  String[]  arraySSMediaIDPos,  String[][]  retTitle2,  Dispatch  objectSheet1,  FargloryUtil  exeUtil,  FargloryExcel  exeExcel)throws  Throwable {
        int     intCol            =  0 ;
        int     intCol2         =  0 ;
        int     intPos            =  0 ;
        int     intPos2         =  0 ;
        ...
        for(int  intNoL=0  ;  intNoL<arraySSMediaID.length  ;  intNoL++) {
            // 年累計
            stringTemp              =  exeExcel.getDataFromExcel2(exeUtil.doParseInteger(arraySSMediaIDPos[intNoL]),   intGetDataRow,  objectSheet1) ;    if("null".equals(stringTemp))stringTemp= "" ;
            //System.out.println(stringTitile+"排名1 Col("+arraySSMediaIDPos[intNoL]+")Row("+intGetDataRow+")值("+stringTemp+")------------------------------------") ;
        ...
        for(int  intNoL=0  ;  intNoL<arraySSMediaID.length  ;  intNoL++) {
            ...
            // 排名
            // 1-3 名
            //System.out.println(stringTitile+" 排名2  Col("+intCol+")RowL("+intPutDataRow+")值("+stringValue+")------------------------------------") ;
            intPos  = exeUtil.doParseInteger(exeUtil.doDeleteDogAfterZero (stringValue)) ;
            if(intPos  >  3)  continue ;
            ...
            //
            exeExcel.putDataIntoExcel(intColOrder11,  intRowOrder+intPos,  retTitle2[intPos2][intCol2], objectSheet1) ;                     stringColName  =  exeExcel.getExcelColumnName( "A",  intCol) ;
            if(",業績,新來人,總來人,".indexOf(stringTitile)            !=-1) stringTemp  =  "="+stringColName+""+(intGetDataRow+1)+"/AG"+(intGetDataRow+1) ;
            if(",成交費用率,來人費用,".indexOf(stringTitile) !=-1) stringTemp  =  "="+stringColName+""+(intGetDataRow+1) ;
            //System.out.println(stringTitile+" 名次 Col("+intColOrder12+")Row("+(intRowOrder+intPos)+")值("+stringTemp+")------------------------------------") ;
    ...
    public  int  getChangeExcelRow(int  intRow)throws  Throwable {
        // 業績     
            if(25  ==  intRow+1)    return 29 ;//本月值25            改為    30
            if(26  ==  intRow+1)    return 30 ;//上月值26            改為    31
            if(28  ==  intRow+1)    return 32 ;//年累計28            改為    33
            if(29  ==  intRow+1)    return 33 ;//年排名29            改為    34
            if(30  ==  intRow+1)    return 34 ;//案累計30            改為    35
        // 新來人
            if(31  ==  intRow+1)    return 38 ;//本月值31            改為    39
            if(32  ==  intRow+1)    return 41 ;//上月值32            改為    42
            if(34  ==  intRow+1)    return 45 ;//年累計34            改為    46
            if(35  ==  intRow+1)    return 48 ;//年排名35            改為    49    由JAVA 處理
            if(36  ==  intRow+1)    return 49 ;//案累計36            改為    50
        // 總來人
            if(37  ==  intRow+1)    return 52 ;//本月值37            改為    53
            if(38  ==  intRow+1)    return 55 ;//上月值38            改為    56
            if(40  ==  intRow+1)    return 59 ;//年累計40            改為    60
            if(41  ==  intRow+1)    return 62 ;//年排名41            改為    63    由JAVA處理
            if(42  ==  intRow+1)    return 63 ;//案累計42            改為    64
        // 排名
            if(61  ==  intRow+1)    return 100 ;
            if(62  ==  intRow+1)    return 101 ;
            if(63  ==  intRow+1)    return 102 ;
        return  intRow ;
    }
    ...
    public  void  doExcel2(int[]  arrayRow,  Dispatch  objectSheet1,  FargloryExcel  exeExcel,  FargloryUtil  exeUtil,  talk  dbAO,  talk  dbDoc,  talk  dbSale)throws  Throwable {
        ...
        for(int  intNo=1  ;  intNo<=exeUtil.doParseInteger(stringMonth)  ;  intNo++) {
            ...
            retTableData            =  dbSale.queryFromPool("speMakerSale03R510B_DATA  '"+stringProjectID1+"',  '"+stringYearAC+"',  '"+stringMonthL+"' " )  ;    // 業績
    ...
    public  void  doExcel8(Hashtable  hashtableDataPosition,  Dispatch  objectSheet1,  FargloryExcel  exeExcel,  FargloryUtil  exeUtil)throws  Throwable {
        ...
        String    stringValue          =  "" ;
        String    stringValue2         =  "" ;
        String[][]  retDataPosition  =  (String[][])  hashtableDataPosition.get("SHEET8") ;   if(retDataPosition  ==  null) return ;
        for(int  intNo=0  ;  intNo<retDataPosition.length  ;  intNo++) {
            intCol          =  exeUtil.doParseInteger(retDataPosition[intNo][0]) ;
    ...
    public  void  doExcel9(Hashtable  hashtableDataPosition,  Dispatch  objectSheet1,  FargloryExcel  exeExcel,  FargloryUtil  exeUtil)throws  Throwable {
        ...
        String    stringValue          =  "" ;
        String    stringValue2         =  "" ;
        String[][]  retDataPosition  =  (String[][])  hashtableDataPosition.get("SHEET9") ;   if(retDataPosition  ==  null) return ;
        for(int  intNo=0  ;  intNo<retDataPosition.length  ;  intNo++) {
            intCol          =  exeUtil.doParseInteger(retDataPosition[intNo][0]) ;
            intRow          =  exeUtil.doParseInteger(retDataPosition[intNo][1]) ;
            stringValue      =  retDataPosition[intNo][2] ;
    ...
    public  String[]  getCostIDDoc7M070(String[]  arraySSMediaID,  FargloryUtil  exeUtil,  talk  dbDoc) throws  Throwable {
        ...
        for(int  intNo=0  ;  intNo<arraySSMediaID.length  ;  intNo++) {
            ...
            for(int  intNoL=0  ;  intNoL<vectorDoc7M070.size()  ;  intNoL++) {
                hashtableDoc7M070  =  (Hashtable)  vectorDoc7M070.get(intNoL) ;  if(hashtableDoc7M070  ==  null)  continue ;
    ...
    public  String  getCostIDMoneyTypeAC(FargloryUtil  exeUtil,  talk  dbDoc) throws  Throwable {
        ...
        for(int  intNo=0  ;  intNo<vectorDoc7M070.size()  ;  intNo++) {
            hashtableDoc7M070  =  (Hashtable)  vectorDoc7M070.get(intNo) ;  if(hashtableDoc7M070  ==  null)  continue ;
            stringCostID               =  ""+hashtableDoc7M070.get("CostID") ;
            stringCostID1              =  ""+hashtableDoc7M070.get("CostID1") ;
            //
    ...
    public  Hashtable  getMoneyFrontTable1(String[]  arraySSMediaIDPos,  String[]  arraySSMediaID,  String[][]  retSale03R510B,  FargloryUtil  exeUtil,  talk  dbDoc,  talk  dbDocCS,  Vector  vectorCostID) throws  Throwable {
        String        stringProjectID1          =  getValue("ProjectID1").trim() ;
        String      stringYearAC            =  getValue("YearAC").trim() ;
        String      stringMonth                 =  getValue("Month").trim() ;
        ...
        for(int  intNo=0  ;  intNo<=exeUtil.doParseInteger(stringMonth)  ;  intNo++) {
            ...
            //
            //System.out.println(intCol+":"+intRow+"KEY("+stringKEY+")("+convert.FourToFive(stringValue,  4)+")------------------------------") ;
            arrayTemp=new  String[3] ;  arrayTemp[0] =  ""+intCol ;  arrayTemp[1] =  ""+intRow ;  arrayTemp[2] =  stringValue ;  vectorData.add(arrayTemp) ;
        }
        // 年累計
        arrayTemp=new  String[3] ;  arrayTemp[0] =  "28" ;  arrayTemp[1] =  ""+intRow ;  arrayTemp[2] =  convert.FourToFive(""+doubleYear,  0) ;  vectorData.add(arrayTemp) ;
        // 案累計
        stringValue  =  ""+hashtableMoney.get("A%-%002") ;
        arrayTemp=new  String[3] ;  arrayTemp[0] =  "31" ;  arrayTemp[1] =  ""+intRow ;  arrayTemp[2] =  stringValue ;                                                vectorData.add(arrayTemp) ;
    ...
    public  Hashtable  getMoneyFrontTable2(int[]  arrayRow,  Vector  vectorTitles,  String[]  arraySSMediaIDPos,  String[]  arraySSMediaID,  String[][]  retSale03R510B,  FargloryUtil  exeUtil,  talk  dbDoc,  talk  dbDocCS,  Vector  vectorCostID) throws  Throwable {
        String        stringProjectID1          =  getValue("ProjectID1").trim() ;
        String      stringYearAC            =  getValue("YearAC").trim() ;
        String      stringMonth                 =  getValue("Month").trim() ;
        String      stringDateStart           =  "" ;
        ...
        doCetMoneyTable2("003",  stringProjectID1,  stringDateStart,  stringDateEnd,  stringCostIDMoneyTypeAC,  stringCostID1To6Media,  stringCostID1To11Media,  arraySSMediaID,  arrayCostID,    retSale03R510B,  exeUtil,  dbDoc,  dbDocCS,  hashtableMoney,  vectorCostID) ;
        // 至本月底   案累計                     全     A%-%004%-%通路代碼(請款代碼)
        // 至本月底   案累計                     純     B%-%004%-%通路代碼(請款代碼)
        System.out.println("案累計---------------------------------S") ;
        doCetMoneyTable2("004",  stringProjectID1,  "",                       stringDateEnd,  stringCostIDMoneyTypeAC,  stringCostID1To6Media,  stringCostID1To11Media,  arraySSMediaID,  arrayCostID,   retSale03R510B,  exeUtil,  dbDoc,  dbDocCS,  hashtableMoney,  vectorCostID) ;
        ...
        for(int  intMode=0  ;  intMode<arrayMode.length  ;  intMode++) {
            ...
            for(int  intNo=0  ;  intNo<arraySSMediaID.length  ;  intNo++) {
                ...
                if(exeUtil.doParseDouble(stringValue)  ==  0)  stringValue="0" ;
                //
                System.out.println("("+intNo+")"+arraySSMediaIDPos[intNo]+":"+intRowL+" 本月值 KEY("+stringKEY+")("+vectorTitles.get(intNo)+")("+convert.FourToFive(stringValue,  4)+")------------------------------") ;
                arrayTemp=new  String[3] ;  arrayTemp[0] =  arraySSMediaIDPos[intNo] ;  arrayTemp[1] =  ""+intRowL ;  arrayTemp[2] =  stringValue ;  vectorData.add(arrayTemp) ;
            }
            // 差異
            doubleYear  =  0 ;
            ...
            arrayTemp=new  String[3] ;  arrayTemp[0] =  "3" ;  arrayTemp[1] =  ""+intRowL ;  arrayTemp[2] =  convert.FourToFive(""+doubleYear,  4) ;  vectorData.add(arrayTemp) ;
        }
        // 資料處理 70 上月值-通路代碼
        intRow       =  69 ;
        stringDate  =  stringYearAC+stringMonth+"01" ;
        ...
        for(int  intMode=0  ;  intMode<arrayMode.length  ;  intMode++) {
            ...
            for(int  intNo=0  ;  intNo<arraySSMediaID.length  ;  intNo++) {       
                ...
                //
                arrayTemp=new  String[3] ;  arrayTemp[0] =  arraySSMediaIDPos[intNo] ;  arrayTemp[1] =  ""+intRowL ;  arrayTemp[2] =  convert.FourToFive(""+doubleYear,  4) ;  vectorData.add(arrayTemp) ;
            }
            // 差異
            doubleYear         =  0 ;
            stringDateStart   =  stringYearAC+"0101" ;
        ...
        arrayMode[1]  =  "" ;
        for(int  intMode=0  ;  intMode<arrayMode.length  ;  intMode++) {
            stringMode  =  arrayMode[intMode] ;  if("".equals(stringMode))  continue ;
            vectorData  =  new  Vector() ;
            intRow        =  66 ;
            intRowL       =  "A".equals(stringMode) ?  intRow  : intRow+10 ;
            for(int  intNo=0  ;  intNo<vectorCostID.size()  ;  intNo++) {
                ...
                for(int  intNoL=0  ;  intNoL<=exeUtil.doParseInteger(stringMonth)  ;  intNoL++) {
                    ...
                    //
                    intRowL++ ;
                    arrayTemp=new  String[3] ;  arrayTemp[0] =  "0" ;  arrayTemp[1] =  ""+intRowL ;  arrayTemp[2] =  stringCostID ;         vectorData.add(arrayTemp) ;
                    arrayTemp=new  String[3] ;  arrayTemp[0] =  "3" ;  arrayTemp[1] =  ""+intRowL ;  arrayTemp[2] =  stringDate ;               vectorData.add(arrayTemp) ;
                    arrayTemp=new  String[3] ;  arrayTemp[0] =  "5" ;  arrayTemp[1] =  ""+intRowL ;  arrayTemp[2] =  stringValue ;            vectorData.add(arrayTemp) ;
    ...
    // 資料庫 Doc
    public  void  doCetMoneyTable1(String  stringSign,  String  stringProjectID1,  String  stringDateStart,  String  stringDateEnd,  FargloryUtil  exeUtil,  talk  dbDoc,  talk  dbDocCS,  Hashtable  hashtableMoney) throws  Throwable {
        String      stringSql                     =  "" ;
        ...
        stringSql    =  " SELECT  SUBSTRING(CONVERT(char(8),UseDate,111),1,7),  SUM(RealMoney) \n"  +
                    ...
                    " WHERE   ( RTRIM(CostID)+RTRIM(CostID1)  IN  (SELECT  RTRIM(CostID)+RTRIM(CostID1) \n"  +
                                                             ...
                                                             " WHERE  BudgetID  LIKE  'B%' \n" +
                                                              " AND  RTRIM(CostID)+RTRIM(CostID1) NOT IN ('871',  '900',  '910') "  +
                                                                " AND  ComNo  =  'CS' "  +
                                                            " ) \n"  +
                                //" OR \n" +
        ...
        if(!"".equals(stringDateStart))  stringSql    +=    " AND  UseDate  >=  '"+stringDateStart+"' \n" ;
        if(!"".equals(stringDateEnd))   stringSql    +=   " AND  UseDate  <=  '"+stringDateEnd+"' \n" ;
    ...
    public  void  doDataTable1(String  stringSign,  String[][]  retData,  FargloryUtil  exeUtil,  Hashtable  hashtableMoney) throws  Throwable {
        ...
        for(int  intNo=0  ;  intNo<retData.length  ;  intNo++) {
            ...
            //
            if(stringSign.endsWith("002"))  stringKEY  =  stringSign ;
            //
            doubleTemp  =  exeUtil.doParseDouble(stringMoney)/10000+exeUtil.doParseDouble(""+hashtableMoney.get(stringKEY)) ;
            //System.out.println("KEY("+stringKEY+")("+convert.FourToFive(""+doubleTemp,  4)+")------------------------------") ;
            hashtableMoney.put(stringKEY,  convert.FourToFive(""+doubleTemp,  4)) ;
        }
    }
    public  void  doCetMoneyTable2(String  stringSign,  String  stringProjectID1,  String  stringDateStart,  String  stringDateEnd,  String  stringCostIDMoneyTypeAC,  String  stringCostID1To6Media,  String  stringCostID1To11Media,  String[]  arraySSMediaID,  String[]  arrayCostID,  String[][]  retSale03R510B,  FargloryUtil  exeUtil,  talk  dbDoc,  talk  dbDocCS,  Hashtable  hashtableMoney,  Vector  vectorCostID) throws  Throwable {
        String      stringSql                     =  "" ;
        String[][]  retData               =  new String[0][0]  ;
        // 2015-12-01  B3018 781 及 782 輸入日期 2015-12-01後不列入處理 
        stringDateStart  =  exeUtil.getDateConvert(stringDateStart) ;
        ...
        stringSql  +=    " GROUP BY CostID,  CostID1,  SSMediaID,  SUBSTRING(CONVERT(char(8),UseDate,111),1,7) \n" ;
          retData  =  dbDoc.queryFromPool(stringSql) ;
        doData2(stringSign,  "A",  stringCostIDMoneyTypeAC,  stringCostID1To6Media,  stringCostID1To11Media,  retData,  arraySSMediaID,  arrayCostID,  retSale03R510B,  exeUtil,  hashtableMoney,  vectorCostID) ;
        System.out.println(stringSign+"全---------------------------------------純S") ;
        doData2(stringSign,  "B",  stringCostIDMoneyTypeAC,  stringCostID1To6Media,  stringCostID1To11Media,  retData,  arraySSMediaID,  arrayCostID,  retSale03R510B,  exeUtil,  hashtableMoney,  vectorCostID) ;
    ...
    public  void  doData2(String  stringSign,  String  stringCostIDMoneyMode,  String  stringCostIDMoneyTypeAC,  String  stringCostID1To6Media,  String  stringCostID1To11Media,  String[][]  retData,  String[]  arraySSMediaID,  String[]  arrayCostID,  String[][]  retSale03R510B,  FargloryUtil  exeUtil,  Hashtable  hashtableMoney,  Vector  vectorCostID) throws  Throwable {
        String    stringCostID              =  "" ;
        String    stringCostID1           =  "" ;
        String    stringYYMM            =  "" ;
        String    stringMoney             =  "" ;
        ...
        String[]    arrayRatio                  =  new  String[arraySSMediaID.length] ;  for(int  intNo=0  ;  intNo<arrayRatio.length  ;  intNo++)                     arrayRatio[intNo]                =  "1" ;
        String[]    arrayRatio1To6Media   =  new  String[6] ;                     for(int  intNo=0  ;  intNo<arrayRatio1To6Media.length  ;  intNo++)   arrayRatio1To6Media[intNo]       =  "0" ;
        String[]    arrayRatio1To11Media    =  new  String[arraySSMediaID.length] ; for(int  intNo=0  ;  intNo<arrayRatio1To11Media.length  ;  intNo++) arrayRatio1To11Media[intNo]    =  "0" ;
        double    doubleTemp            =  0 ;
        ...
        for(int  intNo=0  ;  intNo<retSale03R510B.length  ;  intNo++) {
            ...
            if("B.業績.案累計".equals(stringItem)) {
                ...
                for(int  intNoL=1  ;  intNoL<=6  ;  intNoL++) {
                    ...
                    if(exeUtil.doParseDouble(arrayRatio1To6Media[intNoL-1])  >  0)  booleanFlag  =  true ;
                }
                if(!booleanFlag) {
                    for(int  intNoL=1  ;  intNoL<=6  ;  intNoL++) {
                        arrayRatio1To6Media[intNoL-1]  =  "1" ;
                ...
                for(int  intNoL=1  ;  intNoL<=11  ;  intNoL++) {
                    ...
                    if(exeUtil.doParseDouble(arrayRatio1To11Media[intNoL-1])  >  0)  booleanFlag  =  true ;
                }
                if(!booleanFlag) {
                    for(int  intNoL=1  ;  intNoL<=11  ;  intNoL++) {
                        arrayRatio1To11Media[intNoL-1]  =  "1" ;
        ...
        for(int  intNo=0  ;  intNo<retData.length  ;  intNo++) {
            ...
            //doubleTempSum  =  0 ;
            //
            if(exeUtil.doParseDouble(stringMoney)  ==  0)  continue ;
            //
            if("B".equals(stringCostIDMoneyMode)) {
                System.out.println(intNo+"stringCostIDMoneyTypeAC("+stringCostIDMoneyTypeAC+")stringCostIDMoneyMode("+stringCostIDMoneyMode+")SSMediaID("+stringSSMediaID+")KEY("+stringCostID+stringCostID1+"%-%"+stringYYMM+")("+convert.FourToFive(stringMoney,  4)+")------------------------------0") ;
            }
            if("".equals(stringSSMediaID)) {
                if("B".equals(stringCostIDMoneyMode)  &&  stringCostIDMoneyTypeAC.indexOf(stringCostID+stringCostID1)==-1)  continue ;
                //booleanFlagL  =  false ;
            } else {
                ...
                //System.out.println(intNo+"KEY("+stringCostID+stringCostID1+"%-%"+stringYYMM+")("+convert.FourToFive(stringMoney,  4)+")------------------------------1") ;
                //booleanFlagL  =  true ;
            }
            //if("B".equals(stringCostIDMoneyMode))System.out.println(intNo+"SSMediaID("+stringSSMediaID+")KEY("+stringCostID+stringCostID1+"%-%"+stringYYMM+")("+convert.FourToFive(stringMoney,  4)+")------------------------------1") ;
            //
            if("".equals(stringSSMediaID)) {
            ...
            if("".equals(stringSSMediaID)) {
                ...
                if(stringCostID1To11Media.indexOf(","+stringCostID+stringCostID1+",")  !=  -1) {
                    ...
                    if(vectorCostID.indexOf(stringSSMediaID)  == -1)  vectorCostID.add(stringSSMediaID) ;
                    //
            ...
            for(int  intNoL=0  ;  intNoL<arraySSMediaIDL.length  ;  intNoL++) {
                ...
                if(stringYYMM.startsWith("2016")) {
                    ...
                    //System.out.println(intNo+"("+stringKEY+")("+convert.FourToFive(stringMoney,  0)+")("+convert.FourToFive(""+doubleTempSum,  0)+")------------------------------3") ;
                }
                //if("B".equals(stringCostIDMoneyMode))System.out.println(intNo+"CostID("+stringCostID+")CostID1("+stringCostID1+") KEY("+stringKEY+")("+convert.FourToFive(""+doubleTemp,  4)+")("+convert.FourToFive(""+doubleTempSum,  4)+")------------------------------3") ;
                //if("H".equals(arraySSMediaIDL[intNoL])  &&  "B".equals(stringCostIDMoneyMode)) {
                    System.out.println("("+intNo+"--"+intNoL+")CostID("+stringCostID+")CostID1("+stringCostID1+") stringYYMM("+stringYYMM+")KEY("+stringKEY+")THISMoney("+stringMoney+")("+convert.FourToFive(""+doubleTemp,  4)+")------------------------------3") ;
                //}
                hashtableMoney.put(stringKEY,  convert.FourToFive(""+doubleTemp,  4)) ;
                //
            }
