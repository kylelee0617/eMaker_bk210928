...
public class FuncPrint_R280_AO5 extends bTransaction{
  ...
  public boolean isBatchCheckOK() throws Throwable {
	...
	public boolean action(String value)throws Throwable{
	  ...
	  if( !this.isBatchCheckOK() ) return false;
		...
		// 日期處理
		if(stringEnougDate1.length() > 0) setValue("EnougDate1", fgUtil.getDateAC(stringEnougDate1,"補足日期"));
		if(stringEnougDate2.length() > 0) setValue("EnougDate2", fgUtil.getDateAC(stringEnougDate2,"補足日期"));
		if(stringDateCheck1.length() > 0) setValue("DateCheck1", fgUtil.getDateAC(stringDateCheck1,"合約會審"));
		if(stringDateCheck2.length() > 0) setValue("DateCheck2", fgUtil.getDateAC(stringDateCheck2,"合約會審"));
		if(stringDateRange1.length() > 0) setValue("DateRange1", fgUtil.getDateAC(stringDateRange1,"簽約金到期"));
		if(stringDateRange2.length() > 0) setValue("DateRange2", fgUtil.getDateAC(stringDateRange2,"簽約金到期"));
		if(stringBuyerDate1.length() > 0) setValue("BuyerDate1", fgUtil.getDateAC(stringBuyerDate1,"成交日期"));
		if(stringBuyerDate2.length() > 0) setValue("BuyerDate2", fgUtil.getDateAC(stringBuyerDate2,"成交日期"));
		if(stringEDate1.length() > 0) setValue("EDate1", fgUtil.getDateAC(stringEDate1,"訂單日期"));
		if(stringEDate2.length() > 0) setValue("EDate2", fgUtil.getDateAC(stringEDate2,"訂單日期"));
		if(stringSellerCashDate1.length() > 0) setValue("SellerCashDate1", fgUtil.getDateAC(stringSellerCashDate1,"賣方兌現日"));
		if(stringSellerCashDate2.length() > 0) setValue("SellerCashDate2", fgUtil.getDateAC(stringSellerCashDate2,"賣方兌現日"));
		if(stringBuyerCashDate1.length() > 0) setValue("BuyerCashDate1", fgUtil.getDateAC(stringBuyerCashDate1,"買方兌現日"));
		if(stringBuyerCashDate2.length() > 0) setValue("BuyerCashDate2", fgUtil.getDateAC(stringBuyerCashDate2,"買方兌現日"));
		//
		...
		String retData[][] = null;
		//
		long  longTime1  =  fgUtil.getTimeInMillis( );
		//
    stringSQL = " speMakerSale01R280_AO5_COM2 " +
              "'" + stringOrderDate1 + "'," +
              "'" + stringOrderDate2 + "'," +
              "'" + stringEnougDate1 + "'," +
              "'" + stringEnougDate2 + "'," +
    ...
    retData = dbSale.queryFromPool(stringSQL);
		...
		" from AO_DayPerReportTempShow where (Date_Str between '"+strTypeDate1+"' and '"+strTypeDate2+"') group by [AgentDEPT4] order by [AgentDEPT4] ";
		...
		//
		long  longTime2  =  fgUtil.getTimeInMillis( );
