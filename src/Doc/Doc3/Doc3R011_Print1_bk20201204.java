...
//
public class Doc3R011_Print1_bk20201204 extends bproc{
    ...
    public  void  doExcel(String  stringThisFunction,  FargloryUtil  exeUtil,  talk  dbDoc,  talk  dbFE3D,  talk  dbFED1,  talk  dbAsset) throws  Throwable {
        ...
        vectorDoc3M012    =  exeUtil.getQueryDataHashtable(booleanSource ?  "Doc3M012" : "Doc5M012",   new  Hashtable(),  " AND  BarCode  =  '"+stringBarCode+"' ORDER BY  RecordNo ",  dbDoc) ;    if(vectorDoc3M012.size()==0)  return ;
        stringExcelFileName =  stringBarCode.startsWith("Z")?"請購單-承辦列印2.xlt":"請購單-承辦列印.xltx" ;
        intSheetName      =  "A".equals(stringPrintType) ? 1 : 2 ;        
        //
        FargloryExcel   exeExcel            =  new  FargloryExcel() ; if( stringThisFunction.indexOf("採購")==-1) exeExcel.setVisibleProperty(false) ;
        ...
        if(!(new  File(stringFilePath)).exists()) {
            //stringFilePath  =  "https://emaker11.farglory.com.tw/servlet/baServer3?step=6?filename="+stringFilePath ;
        ...
        /*
        //2018/03/08 - 增加列印法令遵循檢核
        exeExcel.putDataIntoExcel(0,  29,  "法令遵循檢核：1.利害關係人交易:□否□是，檢附:□利害關係人查詢記錄\n 　　　　　　　2.委外作業事項:　□否□是，檢附:□廠商評鑑表",        objectSheet1) ;
        */
        
        if(stringThisFunction.indexOf("請購申請書-簽核彙總")  ==  -1)  messagebox("列印完成。\n(改成A4雷射列印。)") ;
        // 刪除
        //stringSheetName =  "A".equals(stringPrintType) ? "sheet2" : "sheet1" ;
        //objectSheet1      =  Dispatch.call(objectSheets,  "Item",  stringSheetName).toDispatch() ;
...
/*        if(stringThisFunction.indexOf("承辦")  !=  -1)          return  "A" ;   // 方案1及方案2
        ...
        return  (vectorFactoryNo.size()  <=  3) ?  "A" : "B" ;                  */
        return  "A" ;
    }
    ...
    public  void  doPrintDoc3M011(boolean  booleanSource,  String  stringPrintType,  Vector  vectorDoc3M011,  Vector  vectorDoc3M012,  Dispatch  objectSheet1,  FargloryExcel  exeExcel,  FargloryUtil  exeUtil,  talk  dbDoc,  talk  dbFE3D,  talk  dbFED1,  talk  dbAsset) throws  Throwable {
        ...
        //exeExcel.putDataIntoExcel(0,  2,  "單位（案別）："+stringDepartNo+" "+stringDepartName,        objectSheet1) ;
        // 申購名稱
        ...
        exeExcel.putDataIntoExcel(0,  6,  arrayApplyType[0]+"1.預算內"+arrayApplyType[1]+"2.追加"+arrayApplyType[2]+"3.變更"+arrayApplyType[3]+"4.固定資產"+arrayApplyType[4]+"5.其它："+stringApplyTxt,        objectSheet1) ;
        ...
        stringCheckValue  =  "檢附："+arrayCheckAdd[0]+"圖面"   +arrayCheckAdd[1]+"施工規範"+arrayCheckAdd[2]+"材料規範"+arrayCheckAdd[3]+"補充說明 \n"+
                          "      "+arrayCheckAdd[4]+"進度表"+arrayCheckAdd[5]+"報價單"   +arrayCheckAdd[6]+"合約書  "  +arrayCheckAdd[7]+"其它："+stringCheckAddDescript ;
        exeExcel.putDataIntoExcel(8,  "B".equals(stringPrintType)?18:17,  stringCheckValue,        objectSheet1) ;
        // 承辦人員
        //String  stringOriEmployeeNo  =  exeUtil.getVectorFieldValue(vectorDoc3M011,  0,  "OriEmployeeNo") ;
        //String  stringEmpName    =  exeUtil.getNameUnion("EMP_NAME",  "FE3D05",  " AND  EMP_NO  =  '" +  stringOriEmployeeNo  +  "'",  new  Hashtable(),  dbFE3D) ;
    ...
    public  String  getPrintBudgetName(boolean  booleanSource,  String  stringPrintType,  Vector  vectorDoc3M011,  Vector  vectorDoc3M012,  FargloryUtil  exeUtil,  talk  dbDoc,  talk  dbAsset) throws  Throwable {
        ...
        if(!"".equals(stringFilter)) {
            ...
            if(booleanSource) {
                ...
                stringBudgetName  =  exeUtil.getNameUnion("DESCRIPTION",  "Doc7M052",  " AND  RTRIM(CostID1)+RTRIM(CostID2)+RTRIM(CostID3)  =  '" +  exeUtil.doSubstring(stringCostID,0,5)  +  "' ",  new  Hashtable(),  dbDoc) ;
    ...
    // 請購項目
    public  void  doPrintDoc3M012(boolean  booleanSource,  String  stringPrintType,  String  stringThisFunction,  Vector  vectorDoc3M011,  Vector  vectorDoc3M012,  Dispatch  objectSheet1,  FargloryExcel  exeExcel,  FargloryUtil  exeUtil,  talk  dbDoc,  talk  dbFED1,  talk  dbAsset) throws  Throwable {
        String      stringCostID          =  "" ;
        ...
        for(int  intNo=0  ;  intNo<vectorDoc3M012.size()  ;  intNo++) {
            ...
            stringCostIDDetail        =  exeUtil.getVectorFieldValue(vectorDoc3M012,  intNo,  "CostIDDetail") ;   if("null".equals(stringCostIDDetail))  stringCostIDDetail  =  "" ;
            //
            ...
            if(!"".equals(stringFILTER)) {
              ...
              /*
                stringClassName  =  exeUtil.doSubstring(stringCostIDDetail,  0,  4)+"-"+exeUtil.getNameUnion("DESCRIPTION",  "Doc2M022",  " AND  CostIDDetail  =  '"+stringCostIDDetail+"' ",  new  Hashtable(),  dbDoc) ;
              */
                //20180223 修正工料名稱內容列印資料邏輯     
                String costIDTemp = exeUtil.doSubstring(stringCostIDDetail,  0,  4)+"-"+exeUtil.getNameUnion("DESCRIPTION",  "Doc2M022",  " AND  CostIDDetail  =  '"+stringCostIDDetail+"' ",  new  Hashtable(),  dbDoc);
    ...
    public  String  getFactoryNoTel(Vector  vectorDoc3M015,  FargloryUtil  exeUtil) throws  Throwable {
        ...
        if(!"".equals(stringContactTel)) {
            stringTemp      =  exeUtil.getVectorFieldValue(vectorDoc3M015,  0,  "CONTACT_TEL_NO1_PURCHASE") ;       if(!"".equals(stringTemp))  stringContactTel  =  stringTemp  +  "-" + stringContactTel ;
            stringTemp      =  exeUtil.getVectorFieldValue(vectorDoc3M015,  0,  "CONTACT_TEL_NO3_PURCHASE") ;       if(!"".equals(stringTemp))  stringContactTel  =  stringContactTel  +  "#" +  stringTemp ;
        ...
        if(!"".equals(stringContactTel)) {
            stringTemp      =  exeUtil.getVectorFieldValue(vectorDoc3M015,  0,  "COMPANY_TEL_NO1_PURCHASE") ;       if(!"".equals(stringTemp))  stringContactTel  =  stringTemp  +  "-" + stringContactTel ;
            stringTemp      =  exeUtil.getVectorFieldValue(vectorDoc3M015,  0,  "COMPANY_TEL_NO3_PURCHASE") ;       if(!"".equals(stringTemp))  stringContactTel  =  stringContactTel  +  "#" +  stringTemp ;
            return  stringContactTel ;
        }
        return stringContactTel ;
    }
    ...
    public  String[][]  getDoc2M020(String  stringComNo,  String  stringCostID,  String  stringCostID1,  talk  dbDoc) throws  Throwable {
        ...
        stringSql  =  "SELECT  BudgetID,  CostID,  CostID1,  Description "  +
                   " FROM  Doc2M020 "  +
    ...
    public  String  getCost3Name(String  stringCostID,  talk  dbDoc) throws  Throwable {
        ...
        stringSql  =  "SELECT  DESCRIPTION "  +
                     " FROM  Doc7M052 "  +
                   " WHERE  RTRIM(CostID1)+RTRIM(CostID2)+RTRIM(CostID3)  =  '" +  stringCostID  +  "' " ;
    ...
    public  String  getFilterName(String  stringFILTER) throws  Throwable {
        ...
        //
        stringSql  =  " SELECT  F3_NAME "  + 
                  " FROM  AS_ASSET_FILTER " +
                   " WHERE  FILTER  =  '" +  stringFILTER  +  "' " ;
